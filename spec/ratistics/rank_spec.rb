  ...
  describe Rank do
    ...
    context '#cumulative_distribution_function' do
      ...
      it 'returns zero for a nil sample' do
        Rank.cdf(nil, 2).should eq 0
      end

      it 'returns zero for an empty sample' do
        Rank.cdf([].freeze, 2).should eq 0
      end

      it 'returns the probability of the given rank' do
        (0..5).each do |rank|
          probability = Rank.cdf(sorted_sample, rank)
      ...
      it 'returns the probability on an unsorted sample' do
        (0..5).each do |rank|
          probability = Rank.cdf(unsorted_sample, rank)
          probability.should be_within(0.001).of(cdf_values[rank])
        end
      end

      it 'returns the probability on a relatively flat sample' do
        probability = Rank.cdf(flat_sample,2)
      ...
      it 'does not attempt to sort when a using a block' do
        ...
        Rank.cdf(sample, 2, :sorted => false) {|item| item[:count] }
      end
      ...
      context 'with ActiveRecord', :ar => true do
        ...
        specify do
          ...
          probability = Rank.cdf(sample, 38){|r| r.age}
      ...
      context 'with Hamster' do
        ...
        specify do
          probability = Rank.cdf(list, 2, :sorted => true)
          probability.should be_within(0.001).of(0.6)
        end

        specify do
          probability = Rank.cdf(vector, 2, :sorted => true)
          probability.should be_within(0.001).of(0.6)
        end
      end
